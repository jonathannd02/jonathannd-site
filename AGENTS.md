...
**Jonathan Núñez Dhondt · De Atlas** is a small, static personal field atlas for software, security, systems, and practical web work. The site copy and document language are Dutch (`lang="nl"`). It is an Astro single-page site, not a server application: there are no API routes, persistence, database, or framework state layer.

`DESIGN_BRIEF.md` is the living design/technical brief for the **implemented** site. Keep it aligned with the code: one-shot hero video, Canvas 2D Bayer dither on hero + about portrait, editorial section flow, and progressive enhancement. Do not reintroduce a removed SVG-route/map layer unless the brief and markup are intentionally updated together.

## Architecture & Data Flow

- `src/pages/index.astro` is the `/` route. Its frontmatter owns the local `selectedWork` and `notes` data arrays and renders the hero, work, notes, about, contact, and footer sections with `.map()`.
...
- `src/scripts/site.ts` is the only client behavior. After Astro renders static HTML, it handles hero video completion/fallback, Bayer dither overlays for the hero video and about portrait (Canvas 2D), reduced-motion behavior, pointer-position variables, section reveal, and work-row highlighting using `data-*` attributes.
...
- `src/pages/` — route entrypoints; currently `index.astro`.
...
- `public/media/` — hero video/poster (`atlas.mp4`, `atlas-poster.jpg`) and about-portrait assets (`jonathan-portrait.webp`, `jonathan-portrait.png`).
...
- Preserve the existing async/fallback patterns: one-shot video `ended`/`error` listeners; `void video.play().catch(completeHero)` for rejected autoplay; passive pointer events throttled with `requestAnimationFrame`; Canvas dither loops canceled under `prefers-reduced-motion`; `IntersectionObserver` with a reduced-motion and unsupported-browser fallback.
...
- `src/pages/index.astro` — page content, local data, semantic markup, portrait markup, and client-script mount.
- `src/layouts/SiteLayout.astro` — document metadata, favicon, global CSS import, and page slot.
- `src/scripts/site.ts` — media, dither overlays, motion, reveal, pointer, and work-row interactions.
...
- Exercise the progressive-enhancement paths when relevant: poster fallback, video `ended`/`error`/rejected-play handling, reduced-motion immediate completion (and dither disable), IntersectionObserver fallback, hero/portrait dither overlays, and keyboard/pointer work-item highlighting.
